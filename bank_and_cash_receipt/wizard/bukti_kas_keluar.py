...
class BuktiKasKeluarWiz(models.Model):

    _name = 'bukti.kas.keluar.wiz'
    ...
    def default_get(self, fields):
        res = super(BuktiKasKeluarWiz, self).default_get(fields)
    ...
    line_ids = fields.One2many('bukti.kas.keluar.line.wiz', 'bukti_id', copy=True)
    ...
    def create_data(self):
        ...
        for rec in self.line_ids:
            lines.append({'name':rec.name,
                          ...
                          'credit':rec.credit})
        obj = self.env['bukti.kas.keluar'].create({'journal_ids':[(6, 0, self.journal_ids.ids)],
                                                    'date': self.date,
                                                    'period_id':self.period_id.id,
                                                    'ref':self.ref,
                                                    'line_ids':[(0, 0, l) for l in lines]})
        form_view = self.env.ref('bank_and_cash_receipt.view_bukti_kas_kelaur_form')
        return {
            'type': 'ir.actions.act_window',
            'res_model': 'bukti.kas.keluar',
            'view_mode': 'form',
            'view_type': 'form',
...
class BuktiKasKeluarLineWiz(models.Model):

    _name = 'bukti.kas.keluar.line.wiz'
